...
> **Tip:** For large DEG lists, run a lightweight triage pass first to separate genes with any diabetes/islet literature from those with none. Annotate only the former in depth — this cuts runtime by 40–60% on typical lists where half the candidates have no relevant literature.
