...
200 genes stratified into High / Medium / Low in ~40 minutes. Known causal genes (TCF7L2, KCNJ11, SLC30A8) receive High with correct mechanistic summaries and multiple PMIDs. Genes with no functional literature receive Low with "none found" — a falsifiable, citable result rather than a guess.

> **Extending to rare variants:** Replace locus gene names with `GENE:p.VAR` strings and ask the agent to retrieve functional domain context, known pathogenic variants at the same residue, and ClinVar classifications. The same prompt structure applies.
